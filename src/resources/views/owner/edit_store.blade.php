...
<main class="edit-store">
    ...
    <form class="edit-store__form" method="POST" action="{{ $store ? route('owner.update_store', $store->id) : route('owner.store_store') }}">
        @csrf
        @if($store)
        @method('PUT')
        @endif
        ...
        <div class="edit-store__form-group">
            ...
            <input class="edit-store__input" type="text" name="name" value="{{ old('name', $store->name ?? '') }}" required>
        </div>
